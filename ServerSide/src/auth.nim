...
if paramCount() > 0:
  ...
  try:
    ...
    echo "Passed param is not a number, quiting app"
    quit(1);
else:
  lport = CONF["appCONF"]["auth-port"].getInt()
...
proc send_activation_mail(id,email:string){.gcsafe, async.}=
  ...
  let mailtxt = "Thank you for chosing to play with us, here is your link to activate your account \n" &
                #"http://" & CONF["appCONF"]["domain"].getStr() & ":" & $CONF["appCONF"]["port"].getInt() &
                #"/activate?token=" & atoken.toString
                "http://" & CONF["appCONF"]["domain"].getStr() & "activate/?token=" & atoken.toString
...
proc process_requests(req: Request) {.async, gcsafe.} =
  ...
  if req.url.path=="/reg" and req.reqMethod==HttpPost:
    ...
    try:
      ...
      let row = await pgpool.rows(sql"SELECT 1 FROM players WHERE email=?", @[inputed{"email"}.getStr,])
      if row.len > 0:
        errors.add("Email allready in use")
        await req.respond(Http400, $(%*{"error":errors}), rheader)
        return

      #ok so first we check if account with that username allredy exits
      let rown = await pgpool.rows(sql"SELECT 1 FROM players WHERE nick=?", @[inputed{"username"}.getStr,])
      if rown.len > 0:
        errors.add("Username is allready taken by someone else")
        await req.respond(Http400, $(%*{"error":errors}), rheader)
        return
      ...
      let newuserid = await pgpool.rows(sql"INSERT INTO players (nick, email, pass) VALUES (?,?,?) RETURNING id",
                                        @[ inputed["username"].getStr, inputed["email"].getStr, hash_password ])

      #send email with token to new user
      await req.respond(Http200, $(%*{"ok":"Registration successful, check your email."}), rheader)
      ...
      echo ex.msg
      errors.add("Something bad heppend :(, request failed, pls conntact support")
      await req.respond(Http400, $(%*{"error":errors}), rheader)
      return
  ...
  if req.url.path=="/auth" and req.reqMethod==HttpPost:
    ...
    try:
      ...
      #Split to enable login in with username or email
      var is_email_login = false
      if inputed{"username"} == nil:
        is_email_login = true
      ...
      #echo "emejl login: ", is_email_login
      validate_password(inputed, errors)

      echo "finished validating input input"

      if errors.len() > 0:
        await req.respond(Http400, $(%*{"error":errors}), rheader)
        return

      echo "doing postgresql request "

      var gotuser:seq[Row]
      if is_email_login:
        #so if input is ok, we check if email exists and compere inputed and db pass
        gotuser = await pgpool.rows(sql"SELECT players.id, players.pass, players.activ, players.baned, players.score, players.nick FROM players WHERE email = ?", @[inputed["email"].getStr,])
      else:
        #or if we login in with  username
        gotuser = await pgpool.rows(sql"SELECT players.id, players.pass, players.activ, players.baned, players.score, players.nick FROM players WHERE nick = ?", @[inputed["username"].getStr,])

      #let gotuser = await pgpool.rows(sql"SELECT players.id, players.pass, players.activ, players.baned, players.score, players.nick FROM players WHERE email = ?", @[inputed["email"].getStr,])
      #echo "Tim gotusera je: ", typeof(gotuser)
      if gotuser.len>0:#if we found user now need to give him jwtoken so he can use of other services
        ...
        # echo gotuser[0][3] #baned
        # echo gotuser[0][4] #score
        # echo gotuser[0][5] #nick

        echo "finished postgresql"

        echo "verify crypto password"
        ...
        echo "check account activate postgresql again"
        ...
        echo "check finished good"



        #then check if account is baned
        if gotuser[0][3]=="t":
          errors.add("Account baned!")
          await req.respond(Http400, $(%*{"error":errors}), rheader)
          return


        echo "wraping out token"
        var atoken = toJWT(%*{
          "header": {
            "alg": "HS256",
            "typ": "JWT"
        ...
        atoken.sign(secret_jwt)
        echo "token finished sending response"
  ...
  if req.url.path=="/resendemail" and req.reqMethod==HttpPost:
    try:
      ...
      let gotuser = await pgpool.rows(sql"SELECT players.id, players.activ FROM players WHERE email=?",@[inputed["email"].getStr])
      if gotuser.len < 1:
      ...
      if gotuser[0][1] == "t":
        errors.add("Account is allready activated")
        await req.respond(Http400, $(%*{"error":errors}), rheader)
        return

      await send_activation_mail($gotuser[0][0], inputed["email"].getStr)
  ...
  if req.url.path=="/activate" and req.reqMethod==HttpGet and req.url.query[0..5]=="token=":
    ...
    try:
      ...
      if jwtToken.verify(secret_jwt, jwtToken.header.alg ):
        let actid = $jwtToken.claims["activateid"].node.str
        await pgpool.exec(sql"UPDATE players SET activ = true WHERE id=?", @[actid])
      ...
      echo ex.msg
      errors.add("Something bad is happening, pls report issue to admin")
      await req.respond(Http400, $(%*{"error":errors}), rheader)
      return

  #IVALID PATH REQUESTS
  await req.respond(Http404, "Not Found",rheader)

waitFor server.serve(Port(lport), process_requests)