...
proc handshake*(ws: WebSocket, headers: HttpHeaders) {.async.} =
  ...
  if headers.hasKey("Sec-WebSocket-Protocol"):
    #echo wantProtocol
    let wantProtocol:seq[string] = @[ "xxx", ] # headers["Sec-WebSocket-Protocol"].strip()
    var choseProtocol:string

    for p in wantProtocol:
      if p in ws.protocol:
        choseProtocol = ws.protocol
        break

    if ws.protocol != choseProtocol:
...
proc newWebSocket*(req: Request, protocols: seq[string] = @["",]): Future[WebSocket] {.async.} =
